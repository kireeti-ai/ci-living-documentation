...
const handlePushEvent = async (payload: any) => {
  ...
  const commitHash = headCommit?.id || `push-${Date.now()}`
  const metadata: DocumentMetadata = {
    version: '1.0.0',
    branch,
    commit: headCommit?.id || 'unknown',
    commitUrl: headCommit?.url || '',
    branchUrl: `${repoUrl}/tree/${branch}`,
    tags: ['auto-generated', 'push-event'],
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    title: `Push to ${branch}`,
    description: headCommit?.message || 'Push event received',
  }

  try {
    const summary = `# Push Event Received

**Branch:** ${branch}  
**Commit:** ${headCommit?.id?.substring(0, 7) || 'unknown'}  
**Author:** ${headCommit?.author?.name || 'unknown'}  
**Message:** ${headCommit?.message || 'No message'}  

---

*This document was auto-generated by a webhook event.*
...
`
    ...
    console.log(`Created webhook document for ${project.name}: ${commitHash}`)
  } catch (error) {
    console.error('Failed to create webhook document:', error)
  }
}
