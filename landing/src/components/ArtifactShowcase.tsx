...
const visualArtifacts = [
    {
        ...
        src: '/assets/demo/architecture-diagram.png',
        desc: 'Auto-generated component map derived from infrastructure code'
    },
    {
        id: 'erd',
        title: 'Database Schema',
        type: 'ERD (Mermaid)',
        src: '/assets/demo/er-diagram.png',
        desc: 'Real-time schema visualization from SQL/ORM definitions'
    },
    {
        id: 'readme',
        title: 'README Preview',
        type: 'Markdown Output',
        src: '/assets/demo/readme-preview.png',
        desc: 'Production-ready documentation with live status badges'
    },
]
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                ...
                onClick={e => e.stopPropagation()}>
                <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center bg-black/50 text-white hover:bg-black/70 transition-colors z-10">
                    <X size={16} />
                </button>
                <img src={src} alt={title} className="w-full h-full object-contain max-h-[85vh] rounded-lg" />
        ...
        <section id="artifacts" className="section-padding relative">
            ...
            <div className="section-container mt-12">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {visualArtifacts.map((artifact, i) => (
                        <motion.div
                            key={artifact.id}
                            ...
                            className="gh-card overflow-hidden group cursor-pointer"
                            onClick={() => setSelectedImage(artifact.src)}
                        >
                            <div className="aspect-video relative overflow-hidden bg-black/20">
                                ...
                                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
                                    <div className="bg-black/60 backdrop-blur-sm px-3 py-1.5 rounded-full text-xs font-medium text-white flex items-center gap-2 transform translate-y-2 group-hover:translate-y-0 transition-transform">
                                        <FolderOpen size={12} /> View Full
                                    </div>
                                </div>
                            </div>
                            <div className="p-4 border-t border-[var(--border-muted)]">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className="text-[10px] px-1.5 py-0.5 rounded-full font-mono bg-[var(--bg-subtle)] text-[var(--text-muted)] border border-[var(--border-muted)]">
                                        {artifact.type}
                                    </span>
                                </div>
            ...
            <AnimatePresence>
                ...
                {selectedImage && (
                    <ImagePreviewModal
                        src={selectedImage}
                        title={visualArtifacts.find(a => a.src === selectedImage)?.title || 'Preview'}
